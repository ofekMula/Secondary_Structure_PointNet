...
# protein_path for example: Area_1/11as_A/Protein
def save_to_numpy_file(protein_path, array_to_save):
    out_file_name = BASE_DIR + '/' + str.split(protein_path, '/')[1]  # protein name : 11as_A.numpy
...
if __name__ == "__main__":
    ...
    list_of_residues, list_residue_coord = list_of_residue_coordinates_and_residue_seq(chain_prot)
    full = normalize_full_attributes(list_of_residues, list_residue_coord)
    _, list_of_lables = list_of_residue_labels("11as", structure, 'A', chain_prot)
    list_coord, list_lables = fill_arrays_to_num_points(full, list_of_lables)
    print("LEN list_coord:", list_coord.shape)
    print(list_coord)
    print("LEN list_lables:", list_lables.shape)
    print(list_lables)
