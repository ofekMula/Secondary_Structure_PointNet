...
def create_cord_color_array(coordinates, colors):
    if len(coordinates) != len(colors):
        print("ERROR - different number of coordinates and colors")
    else:
        arr = np.array([])
        for i in range(0, len(coordinates)):
            arr = np.array(arr, [coordinates[i], colors[i]])  # add current coordinates with there color
        return arr
